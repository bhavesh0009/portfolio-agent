...
- Builds initial portfolio based on user inputs: investor profile (aggressive/moderate/defensive), deployed capital, risk tolerance
...
The system uses a three-tier architecture for data persistence:
- **Frontend (Next.js)**: Makes API calls to FastAPI backend (no direct database access)
- **Backend (FastAPI)**: REST API server on port 8000, queries Supabase PostgreSQL
- **Database (Supabase PostgreSQL)**: 14 tables for portfolios, stocks, metrics, and analysis

**Key Tables**:
- `portfolios` - Portfolio metadata and configuration
- `stocks` - Stock allocations with entry prices, targets, stop-losses
- `investment_views` - AI-generated investment thesis per stock
- `key_metrics` - Financial metrics from screener.in
- `daily_prices` - Historical price tracking
- `performance_metrics` - Portfolio performance over time

**Migration from SQLite**:
SQLite database has been archived to `.cache/portfolio.db.sqlite_backup_20251226`. All data was migrated to Supabase PostgreSQL with 100% validation match (214 rows across 15 tables).
...
`config.ini` controls:
- Portfolio settings: `max_stocks`, `initial_capital`, `investor_profile` (aggressive/moderate/defensive)
- Allocation strategies: Different percentages for aggressive/moderate/defensive profiles
...
**Industry and Sector are enriched via Supabase PostgreSQL database, not screener.in:**

screener.in does not support "Industry" or "Sector" as queryable or configurable columns. However, the system automatically enriches stock results with industry, sector, and symbol data from a Supabase PostgreSQL database (view: `v_company_industry`) when:
- Industry or Sector is requested in the columns list
- Industry or Sector filters are present in the query

**How it works:**
1. **Query preprocessing**: Industry/Sector filter conditions are detected and removed from the query sent to screener.in
   - Example: `Market Cap > 5000 AND Industry = "Hotels"` becomes `Market Cap > 5000`
2. **Company ID extraction**: `data_row_company_id` attributes are extracted from screener.in table rows
   - Each stock row has a unique company identifier: `<tr data-row-company-id="681">`
3. **Supabase bulk enrichment**: Single PostgreSQL query joins company IDs with industry/sector data
   - Field mapping: `industry_name` → Industry, `category_name` → Sector, `company_identifier` → Symbol
   - Exchange inferred from symbol format (numeric → BSE, alphabetic → NSE)
   - Typical latency: ~200-500ms for 300 stocks (vs. ~60s with previous yfinance approach)
4. **Post-filtering**: Results are filtered by Industry/Sector criteria after enrichment

**Performance characteristics:**
- **Single bulk query** for all stocks (not per-stock API calls)
- **No external API dependencies** (database query, not web API)
- **No rate limiting** (PostgreSQL can handle high query volumes)
- **Graceful degradation**: Sets 'Unknown' for missing company IDs

**Database schema (`v_company_industry` view):**
- `data_row_company_id` (TEXT) - Join key from screener.in
- `company_name` (TEXT) - Company name
- `company_identifier` (TEXT) - Stock ticker symbol (e.g., COALINDIA, BAJAJ-AUTO)
- `category_name` (TEXT) - Sector name
- `industry_name` (TEXT) - Industry name

**Agent usage:**
```python
# This works exactly the same - transparent to agents
screen_stocks(
...
- Enrichment progress: "Enriching 293 stocks via Supabase bulk query..."
- Post-filtering: "Post-filtered: 293 -> 15 stocks"
- Statistics: "Supabase enrichment stats: 278 successful, 15 failed (5.1% failure rate)"
...
**Moderate Profile:**
- Stop-loss: -12% (balanced risk tolerance)
- Target range: 25-40% (moderate growth)
...
**Architecture**: Frontend → FastAPI (port 8000) → Supabase PostgreSQL

The frontend fetches data via REST API calls to the FastAPI backend, which queries Supabase PostgreSQL.

Frontend API Routes (Next.js server-side):
...
- `GET http://localhost:8000/api/portfolios` - All portfolios
- `GET http://localhost:8000/api/portfolios/active` - Active portfolio
- `GET http://localhost:8000/api/portfolios/{id}/stocks` - Portfolio stocks
- `GET http://localhost:8000/api/stocks/{id}` - Stock details
- `GET http://localhost:8000/api/portfolios/{id}/stats` - Portfolio statistics
