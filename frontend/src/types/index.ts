...
export interface Portfolio {
  ...
  cash_balance?: number; // Unallocated cash (NULL for legacy portfolios)
  created_at: string;
  timestamp: string;
...
export interface Stock {
  ...
  shares?: number; // Number of whole shares (NULL for legacy portfolios)
...
export interface PortfolioPerformance {
  portfolio_id: number;
  current_value: number;  // Stock + Cash total
  stocks_value: number;  // Stocks only
  cash_balance: number;  // Cash portion
  initial_capital: number;
  pnl_absolute: number;  // Now includes cash
  pnl_pct: number;  // Now includes cash
  num_stocks: number;
