...
export default function StockCard({ stock, onClick }: StockCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Shares Calculation - Show calculation: shares × price = value */}
      {stock.shares !== undefined && (
        <div className="mb-3 p-2 bg-primary-50 rounded-lg">
          <div className="text-xs text-primary-700 font-medium">
            {stock.shares.toLocaleString()} shares × {formatCurrency(stock.entry_price)} = {formatCurrency(stock.allocation_amount)}
          </div>
        </div>
      )}
