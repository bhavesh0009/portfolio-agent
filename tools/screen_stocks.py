...
from .supabase_enricher import get_enricher
    ...
    with ScreenerSession() as session:
        ...
        if fetch_all_pages:
            ...
            if not table_data or 'rows'  not in table_data:
                ...
                metrics_collector.record_tool_call("screen_stocks", duration_ms)
        ...
        # Always enrich with Symbol/Exchange/Industry/Sector via Supabase
        if len(stocks) > 0:
            enricher = get_enricher()

            logger.separator()
            logger.info(f"[SUPABASE ENRICHMENT] Enriching {len(stocks)} stocks with industry, sector, and symbol data")
            logger.separator()

            # Single bulk enrichment (one database query for all stocks)
            try:
                enricher.enrich_stocks_bulk(stocks)
                logger.info(f"Bulk enrichment complete")
                enricher.log_enrichment_stats()
            except Exception as e:
                logger.error(f"Supabase enrichment failed: {e}")
                # Fallback: Set all stocks to Unknown
                for stock in stocks:
        ...
        if columns:
            ...
            for stock in stocks:
                ...
                # Always preserve internal fields (starting with _)
                for key in stock.keys():
                    if key.startswith('_'):
                        filtered_stock[key] = stock[key]
