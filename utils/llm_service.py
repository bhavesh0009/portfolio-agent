...
class GeminiService:
    ...
    def generate_content(
        ...
        Args:
            ...
            agent_name: Name of calling agent for metrics tracking, optional
        ...
        for attempt in range(max_retries):
            try:
                ...
                call_duration_ms = (time.perf_counter() - call_start_time) * 1000

                # Extract token usage (always, not just when verbose)
                input_tokens = 0
                output_tokens = 0
                if hasattr(response, 'usage_metadata') and response.usage_metadata:
                    ...
                    # Record metrics if agent name provided
                    if agent_name:
                ...
                if verbose:
                    token_info = f" (Tokens: input={input_tokens}, output={output_tokens})"
